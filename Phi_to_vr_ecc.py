...
def plot_2D_frame(dir, plot_dir, start_nr, end_nr, xy=False, r_p=False, dvar='rho', inner_circle=1, outer_circle=10):
    ...
    Z_axis = np.zeros((len(tot_list), len(list)))
    ...
    for i in range(len(list)):
        for j in range(len(tot_list)):
            ...
            mean_Z = np.mean(Z[(((X-x2)**2+(Y-y2)**2)<((outer_circle*lenscale)**2))&(((X-x2)**2+(Y-y2)**2)>((inner_circle*lenscale)**2))])
            
            Z_axis[j, i] = mean_Z
            r_axis[j,i] = np.sqrt(x2**2+y2**2)
            time_axis[j, i] = d['Time']
    ...
    for j in range(len(tot_list)):
        if dvar == 'rho':
            plt.plot(phase_axis[1:-1], Z_axis[j][1:-1], label='e = '+str(eccentricities[j]), color=colors[j])
        if dvar == 'press':
            plt.plot(phase_axis[1:-1], Z_axis[j][1:-1], label='e = '+str(eccentricities[j]), color=colors[j])
        if dvar == 'T':
            plt.plot(phase_axis[1:-1], Z_axis[j][1:-1], label='e = '+str(eccentricities[j]), color=colors[j])
        if dvar == 'vel1':
            plt.plot(phase_axis[1:-1], Z_axis[j][1:-1], label='Gas velocity for e = '+str(eccentricities[j]), color=colors[j])
    ...
    #plt.yscale('log')
    if dvar=='vel1':
        ylabel = '$V_{\mathrm{los}}$ [cm/s]'
    if dvar=='rho':
        ylabel = r'$\rho$ [gr/cm$^3$]'
    if dvar=='press':
        ylabel = r'P [Barye]'
    if dvar=='T':
        ylabel = 'T [K]'
...
plot_2D_frame(dir, plot_dir, start_nr, end_nr, xy=True, r_p=True, dvar='T', inner_circle=1, outer_circle=5)
